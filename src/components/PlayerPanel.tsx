  ...
  return (
    <div className="relative z-0">
      <div
        ...
        aria-hidden
      />
      <section className="panel relative z-0" style={accentColor ? ({ ["--track-accent" as any]: accentColor }) : undefined}>
        <div className="relative z-0 flex flex-col gap-5 overflow-hidden rounded-[inherit] p-5">
          ...
          <div
            ...
            className="group relative overflow-hidden rounded-xl bg-black shadow-lg shadow-black/20 ring-1 ring-white/[0.06] transition-opacity duration-300"
            ...
            {current ? (
              <>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {current.title ? (
                    <h2 className="min-w-0 flex-1 truncate text-2xl font-bold tracking-tight sm:text-3xl">{current.title}</h2>
                  ) : (
                    <h2 className="min-w-0 flex-1 animate-pulse truncate text-2xl font-bold text-muted-foreground/50 sm:text-3xl">Tocando {current.trackId}</h2>
                  )}
                  <ExternalLinkButton url={current.url} />
                </div>
                <p className="mt-1.5 text-sm text-muted-foreground">
                  Pedido por{" "}
                  <span className="font-medium text-foreground" style={current.requesterColor ? { color: current.requesterColor } : undefined}>{current.requestedBy}</span>
          ...
          <div className="flex flex-wrap items-center gap-5">
            <div className="flex items-center gap-2">
              <Button
                size="icon"
                variant="secondary"
                className="size-10 rounded-full border border-white/[0.06] bg-white/[0.03] transition-all duration-200 hover:border-white/[0.1] hover:bg-white/[0.07] active:scale-95"
                onClick={onPrevious}
                disabled={!hasPrevious}
                aria-label="Música anterior"
              ><SkipBack className="size-4" aria-hidden /></Button>
              <Button
                size="icon"
                className="bg-gradient-primary glow size-12 rounded-full text-primary-foreground shadow-lg shadow-primary/25 transition-all duration-200 hover:scale-[1.04] hover:shadow-xl hover:shadow-primary/30 active:scale-95"
                onClick={() => {
                  setPaused((value) => {
                    const nextVal = !value;
                    onTogglePlayChange?.(nextVal);
                ...
                disabled={!current}
                aria-label={paused ? "Tocar" : "Pausar"}
              >{paused ? <Play className="size-5" aria-hidden /> : <Pause className="size-5" aria-hidden />}</Button>
              <Button
                size="icon"
                variant="secondary"
                className="size-10 rounded-full border border-white/[0.06] bg-white/[0.03] transition-all duration-200 hover:border-white/[0.1] hover:bg-white/[0.07] active:scale-95"
              ...
              ><SkipForward className="size-4" aria-hidden /></Button>
            </div>
            ...
            <div className="flex min-w-40 flex-1 items-center gap-3">
              <Button
                size="icon"
                variant="ghost"
                className="size-8 shrink-0 rounded-full transition-colors duration-200 hover:bg-white/[0.05]"
                onClick={() => setMuted((value) => !value)}
                aria-label={muted ? "Tirar do mudo" : "Deixar mudo"}
              >{muted ? <VolumeX className="size-4 text-muted-foreground" aria-hidden /> : <Volume2 className="size-4 text-muted-foreground" aria-hidden />}</Button>
              <Slider
...
function ExternalLinkButton({ url }: { url: string }) {
  return (
    <a href={url} target="_blank" rel="noreferrer" className="text-muted-foreground transition-colors hover:text-primary" aria-label="Abrir no YouTube">
